/**
 * Agentic AI Neural Network Visualization
 *
 * Animated neural network with orbiting stat nodes, pulsing core,
 * and data particle effects using GSAP.
 */
...
/**
 * Initialize the Agentic AI neural network visualization
 */
export function initAgenticNeuralNetwork(): void {
  ...
  gsap.ticker.add(() => {
    nodes.forEach((node, index) => {
      const line = svg.querySelector(
        `.agentic-connection-line[data-node-index="${index}"]`
      ) as SVGLineElement;
      if (!line) return;
...
function drawConnections(
  ...
  // Clear existing
  svg
    .querySelectorAll(".agentic-connection-line, .agentic-data-pulse")
  ...
  positions.forEach((pos, index) => {
    ...
    line.setAttribute("class", "agentic-connection-line");
    line.setAttribute("data-node-index", index.toString());
    line.setAttribute("stroke", "rgba(234, 36, 43, 0.5)");
    ...
    pulse.setAttribute("class", "agentic-data-pulse");
    pulse.setAttribute("data-node-index", index.toString());
    pulse.setAttribute("fill", "#EA242B");
...
function createDataParticles(
  ...
  container.innerHTML = "";
  svg.querySelectorAll(".agentic-particle-line").forEach((el) => el.remove());

  // Create 8 floating particles
  for (let i = 0; i < 8; i++) {
    const particle = document.createElement("div");
    particle.className = "agentic-data-particle";
    ...
    line.setAttribute("class", "agentic-particle-line");
