/**
 * DART AI Neural Network Visualization
 *
...
/**
 * Initialize the DART AI neural network visualization
 */
export function initDartNeuralNetwork(): void {
  ...
  gsap.ticker.add(() => {
    nodes.forEach((node, index) => {
      const line = svg.querySelector(
        `.connection-line[data-node-index="${index}"]`
      ) as SVGLineElement;
      if (!line) return;
...
function drawConnections(
  ...
  // Clear existing
  svg
    .querySelectorAll(".connection-line, .data-pulse")
